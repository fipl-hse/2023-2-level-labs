...
def prepare_word(
    ...
    if not isinstance(raw_word, str) or not isinstance(
            start_of_word, str | None) or not isinstance(
            end_of_word, str | None):
        return None
    chars = list(raw_word)
    if start_of_word is not None:
        chars.insert(0, start_of_word)
    if end_of_word is not None:
        chars.append(end_of_word)
    return tuple(chars)
...
def collect_frequencies(
    ...
    if not isinstance(text, str) or not isinstance(
        ...
        return None
    freq_dict = {}
    for r_word in text.split():
        preproc_word = prepare_word(r_word, start_of_word, end_of_word)
        if preproc_word is None:
            return None
        if preproc_word not in freq_dict:
            freq_dict[preproc_word] = 0
        freq_dict[preproc_word] += 1
    return freq_dict
...
def count_tokens_pairs(
    ...
    for word, freq in word_frequencies.items():
        for i in range(len((word)) - 1):
            if (word[i], word[i + 1]) not in freq_pairs:
                freq_pairs[(word[i], word[i + 1])] = 0
            freq_pairs[(word[i], word[i + 1])] += freq
    return freq_pairs
...
def merge_tokens(
    ...
    :return: dictionary in the form of <preprocessed word: number of occurrences>
    """
    if not isinstance(word_frequencies, dict) or not isinstance(
            pair, tuple):
        return None
    dict_of_freqs = {}
    mer_tok = ''.join(pair)
    for prep_w in word_frequencies:
        listed = list(prep_w)
        for num in range(len(prep_w) - 1):
            mer_pair = ''.join((prep_w[num], prep_w[num + 1]))
            if mer_pair == mer_tok:
                listed[num + 1] = mer_tok
                listed.pop(num)
        dict_of_freqs[tuple(listed)] = word_frequencies[prep_w]
...
def train(
    ...
    :return: dictionary in the form of <preprocessed word: number of occurrences>
    """
    if not isinstance(word_frequencies, dict | None) or not isinstance(
            num_merges, int):
        return None
    while num_merges > 0:
        ...
        if pairs is None:
            return None
        num_merges = min(len(pairs), num_merges)
        for numm in range(num_merges):
