...
def prepare_word(
    ...
    if not isinstance(raw_word, str):
        return None
    if not (isinstance(start_of_word, str) or start_of_word is None):
        return None
    if not (isinstance(end_of_word, str) or end_of_word is None):
        return None
...
def collect_frequencies(
    ...
    for word in list_of_words:
        ...
        if not prepared_word:
            return None
        dict_of_freq[prepared_word] = list_of_words.count(word)
    return dict_of_freq
...
def train(
    ...
    if not dict_with_pairs:
        return None
    if len(dict_with_pairs) > num_merges:
        merges = num_merges
    else:
...
def get_vocabulary(
    ...
    for keys in word_frequencies.keys():
        for token in keys:
            ...
            for symbol in token:
                tokens.add(symbol)
    new_tokens = sorted(tokens).sort(key=len, reverse=True)
    ident_tokens = {}
