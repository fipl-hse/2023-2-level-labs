...
class TextProcessor:
    ...
    def get_token(self, element_id: int) -> Optional[str]:
        ...
        """
        corresponding_storage = {identifier: s for s, identifier in self._storage.items()}

        if not isinstance(element_id, int) or element_id not in corresponding_storage:
            return None

        return corresponding_storage[element_id]
...
class GreedyTextGenerator:
    ...
    def run(self, seq_len: int, prompt: str) -> Optional[str]:
        ...
        for token in range(seq_len):
            next_token = self._model.generate_next_token(encoded_prompt)
            if not next_token:
                break
            best = [key for key, value in next_token.items() if value == max(next_token.values())]
