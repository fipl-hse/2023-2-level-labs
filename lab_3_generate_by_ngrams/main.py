...
class TextProcessor:
    ...
    def _tokenize(self, text: str) -> Optional[tuple[str, ...]]:
        ...
        for token in text.lower():
            if token.isspace() and tokenized_text[-1] != self._end_of_word_token:
                tokenized_text.append(self._end_of_word_token)
            elif token.isalpha():
                tokenized_text.append(token)
        if not text[-1].isalnum():
            tokenized_text.append(self._end_of_word_token)
        if tokenized_text.count(self._end_of_word_token) == len(tokenized_text):
            return None
    ...
    def _put(self, element: str) -> None:
        ...
        if element not in self._storage:
            self._storage[element] = len(self._storage)z
        return None
...
class NGramLanguageModel:
    ...
    def _extract_n_grams(
        ...
        if not isinstance(encoded_corpus, tuple) or not encoded_corpus:
            return None
        list_of_n_grams = []
        for ind in range(len(encoded_corpus) - 1):
            list_of_n_grams.append(tuple(encoded_corpus[ind: ind + self._n_gram_size]))
        return tuple(list_of_n_grams)
